      ...
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden my-8">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Khách hàng: <span className="text-emerald-700 font-semibold">{customer.name}</span> ({customer.phone})
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-full text-slate-500 hover:text-rose-600 bg-slate-100 hover:bg-rose-50 transition cursor-pointer border border-slate-200 shadow-xs"
            title="Đóng"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 bg-slate-50">
          ...
          <div className="space-y-3">
            ...
            {items.map((item, idx) => (
              <div key={idx} className="bg-white p-3 rounded-xl border border-slate-200 space-y-2 shadow-sm">
                <div className="flex items-center space-x-2">
                  <select
                    value={item.productName}
                    onChange={(e) => handleProductChange(idx, e.target.value)}
                    className="flex-1 bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                  >
                  ...
                  <button
                    ...
                    className="p-1.5 text-slate-500 hover:text-rose-600 hover:bg-rose-50 bg-white border border-slate-300 rounded-lg transition shrink-0 cursor-pointer"
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-300 rounded-lg px-2 py-1 text-slate-900 text-xs focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] text-slate-500 mb-0.5">Đơn giá (VND)</label>
                    <input
                      type="number"
                      step={50000}
                      value={item.price}
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center justify-between shadow-xs">
            ...
            <div className="text-lg font-bold text-[#00793d]">
              {formatVND(totalAmount)}
            </div>
          </div>

          {/* Automation Notice */}
          <div className="flex items-start space-x-2 bg-indigo-50 border border-indigo-200 p-3 rounded-xl text-xs text-indigo-700">
            <Sparkles className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
            <div>
              <span className="font-semibold text-indigo-900">Kích hoạt Chăm Sóc Tự Động (Automation):</span>
              <p className="mt-0.5 text-slate-600">
                Sau khi bấm Tạo đơn, CRM sẽ chuyển trạng thái khách sang <strong className="text-indigo-700">Won</strong> và tự động lên lịch gửi tin WhatsApp theo quy trình Ngày +3, +5, +7, +15!
              </p>
            </div>
          </div>
          ...
          <div className="pt-2 flex items-center justify-end space-x-3">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 rounded-lg text-sm text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition"
            >
              Hủy
            </button>
            <button
